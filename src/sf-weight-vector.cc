...
void SfWeightVector::AddVectorAlternate(int id_x, 
  ...
  /*for (int i = 0; i < dimensions_; ++i) {
    if (i == this_x_feature) {
      ...
      weights_[i] += eta * (this_x_value - weights_[i] - base_updates[id_x][i] + avg_updates[closest_center][i]);
      //inner_product += weights_[i] * this_x_value * eta;
      if (++cnt < num_features) this_x_feature = x.FeatureAt(cnt);
    } else {
      weights_[i] += eta * (-weights_[i] - base_updates[id_x][i] + avg_updates[closest_center][i]);
    }
  }*/
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] += eta * (-weights_[i] - base_updates[id_x][i] + avg_updates[closest_center][i]);
  ...
  for (int i = 0; i < num_features; ++i) {
    weights_[x.FeatureAt(i)] += eta * x.ValueAt(i);
  }

  //squared_norm_ *= (1.0 - eta) * (1.0 - eta);
  squared_norm_ = 0.0;
  for (int i = 0; i < dimensions_; ++i) {
    squared_norm_ += weights_[i] * weights_[i];
  }
  //squared_norm_ += x.GetSquaredNorm() * eta * eta + 2.0 * (1.0 - eta) * inner_product;
}

void SfWeightVector::AddMBVectorAlternate(int id_mb, 
                                        const SfSparseVector& x, 
                                        float eta, 
                                        float* mb_avg_updates, 
                                        float** mb_base_updates) {
  if (x.FeatureAt(x.NumFeatures() - 1) > dimensions_) {
    std::cerr << "Feature " << x.FeatureAt(x.NumFeatures() - 1) 
        << " exceeds dimensionality of weight vector: " 
        << dimensions_ << std::endl;
  ...
  int num_features = x.NumFeatures();
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] += eta * (-weights_[i] - mb_base_updates[id_mb][i] + mb_avg_updates[i]);
  }
  for (int i = 0; i < num_features; ++i) {
    weights_[x.FeatureAt(i)] += eta * x.ValueAt(i);
  }
...
void SfWeightVector::AddVectorCompact(/*int closest_center,*/
                                      float eta,
                                      float* pre_weights/*, 
                                      float** avg_updates*/) {
  /*if (x.FeatureAt(x.NumFeatures() - 1) > dimensions_) {
  ...
  //int num_features = x.NumFeatures();
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] += eta * (-weights_[i] + pre_weights[i] /*+ avg_updates[closest_center][i]*/);
  }
...
void SfWeightVector::UpdateWeights(/*int closest_center,*/
                                    float eta, 
                                    const SfSparseVector& x,
                                    float* w, 
                                    /*float** avg_updates,*/
  ...
  if (flag == 0) {
    for (int i = 0; i < dimensions_; ++i) {
      weights_[i] += eta * (-w[i] /*+ avg_updates[closest_center][i]*/);
    }
  ...
  /*else {
    for (int i = 0; i < dimensions_; ++i) {
      weights_[i] += eta * avg_updates[closest_center][i];
    }
  }*/
...
void SfWeightVector::SAGAUpdateWeights(float eta,
                                        float *g) {
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] -= eta * (g[i]);
  }
...
void SfWeightVector::ComputeSquaredNorm() {
  squared_norm_ = 0.0;
  for (int i = 0; i < dimensions_; ++i) {
    squared_norm_ += weights_[i] * weights_[i];
  }
...
void SfWeightVector::AddSVectorCompact(float eta, 
                                       float* pre_weights, 
                                       float* avg_updates) {
  /*if (x.FeatureAt(x.NumFeatures() - 1) > dimensions_) {
    std::cerr << "Feature " << x.FeatureAt(x.NumFeatures() - 1) 
  ...
  //int num_features = x.NumFeatures();
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] += eta * (/*-weights_[i] + pre_weights[i] +*/ avg_updates[i]);
  }
  /*for (int i = 0; i < num_features; ++i) {
    weights_[x.FeatureAt(i)] += eta * x.ValueAt(i);
  }*/
...
void SfWeightVector::AddMBVectorCompact(const SfSparseVector& x, 
                                        float eta, 
                                        float* mb_base_updates,
                                        float* mb_avg_updates) {
  if (x.FeatureAt(x.NumFeatures() - 1) > dimensions_) {
  ...
  int num_features = x.NumFeatures();
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] += eta * (-weights_[i] - mb_base_updates[i] + mb_avg_updates[i]);
  }
  for (int i = 0; i < num_features; ++i) {
    weights_[x.FeatureAt(i)] += eta * x.ValueAt(i);
  }
...
void SfWeightVector::AddMB2VectorCompact(int closest_center, 
                                        const SfSparseVector& x, 
                                        float eta, 
                                        float** mb_avg_updates) {
  if (x.FeatureAt(x.NumFeatures() - 1) > dimensions_) {
  ...
  int num_features = x.NumFeatures();
  for (int i = 0; i < dimensions_; ++i) {
    weights_[i] += eta * (-weights_[i] + mb_avg_updates[closest_center][i]);
  }
  for (int i = 0; i < num_features; ++i) {
    weights_[x.FeatureAt(i)] += eta * x.ValueAt(i);
  }

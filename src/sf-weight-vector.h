...
class SfWeightVector {
 public:
  ...
  // w += x_scale * x
  void AddVectorAlternate(int id_x, 
  ...
  void AddMBVectorAlternate(int id_mb, 
                            const SfSparseVector& x, 
                            float eta, 
                            float* mb_avg_updates, 
                            float** mb_base_updates);

  void AddVectorCompact(/*int closest_center,*/
                        float eta, 
                        float* pre_weights/*, 
                        float** avg_updates*/);

  void UpdateWeights(/*int closest_center,*/
                      float eta, 
                      const SfSparseVector& x,
                      float* w, 
                      /*float** avg_updates,*/
  ...
  void SAGAUpdateWeights(float eta,
                        float* g);

  void AddSVectorCompact(float eta, 
                         float* pre_weights, 
                         float* avg_updates);

  void AddMBVectorCompact(const SfSparseVector& x, 
                                        float eta, 
                                        float* mb_base_updates,
                                        float* mb_avg_updates);

  void AddMB2VectorCompact(int closest_center, 
                                        const SfSparseVector& x, 
                                        float eta, 
                                        float** mb_avg_updates);
  ...
  float* GetWeight() const { return weights_; }
  void ComputeSquaredNorm();

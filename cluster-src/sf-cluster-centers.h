...
class SfClusterCenters {
 public:
  ...
  void UpdatesMemAlloc(long int num_samples, bool fast);
  void InitUpdates();
  void SetUpdates(int id_x, const SfSparseVector& x, int closest_center, bool fast);
  void SetAvgUpdates(int id_center, long int num_examples);
  float** GetAvgUpdates() { return avg_updates_; }
  float** GetBaseUpdates() { return base_updates_; }


  void MBUpdatesMemAlloc(long int num_mb, bool fast);
  void MBInitUpdates(long int num_mb, bool fast);
  void MBSetUpdates(long int id_mb, const SfSparseVector& x, int closest_center, bool fast);
  void MBSetAvgUpdates(long int q, int mini_batch_size, int last_mb, long int num_examples, bool fast);
  float* GetMBAvgUpdates() { return mb_avg_updates_; }
  float** GetMBBaseUpdates() { return mb_base_updates_; }

  void SUpdatesMemAlloc(long int num_samples, bool fast);
  void SInitUpdates();
  void SSetUpdates(int id_x, const SfSparseVector& x, int closest_center, bool fast);
  void SSetAvgUpdates(long int num_examples);

  //void MB2SetAvgUpdates(SfClusterCenters& new_centers, vector<int>& n, int num_examples);
  void MB2SetAvgUpdates(SfClusterCenters& new_centers);
 ...
 protected:
  ...
  float* mb_avg_updates_;
  float** mb_base_updates_;
};
